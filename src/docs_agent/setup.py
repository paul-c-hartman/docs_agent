...
def print_unless_silent(*args, **kwargs):
...
def ensure_directory(directory="."):
    ...
    print_unless_silent(f"Directory exists: {docs_dir}")
    ...
    if not os.path.exists(config_path):
        ...
        print_unless_silent(f"Created local config file: {config_path}")
    else:
        print_unless_silent(f"Local config file already exists: {config_path}")
    if not os.path.exists(elements_path):
        with open(elements_path, "w") as f:
            f.write("# Languages, libraries, frameworks and tools in use\n")
        print_unless_silent(f"Created local elements file: {elements_path}")
    else:
        print_unless_silent(f"Local elements file already exists: {elements_path}")
    ...
    if not os.path.exists(chromadb_dir):
        ...
        print_unless_silent(f"Created ChromaDB directory: {chromadb_dir}")
    else:
        print_unless_silent(f"ChromaDB directory already exists: {chromadb_dir}")
